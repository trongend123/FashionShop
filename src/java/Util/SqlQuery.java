...
//        public static final String getAllCategoryName = "SELECT [CategoryName] FROM [CATEGORIES]";
    ...
    interface cart {
        ...
        public static final String getCartListByOrderId = "select CartID, a.UserID, a.UserName, [Address], Payment, c.Quantity, o.OrderID, OrderDate, ShipDate, ReceivedDate, [Status] from CART c inner join [ORDERS] o on c.OrderID= o.OrderID\n"
                + "inner join ACCOUNT a on c.UserID= a.UserID\n"
                + "inner join SIZE s on s.SizeID= c.SizeID\n"
                + "\n"
                + "where o.OrderID=? and AgentID=?";
    }
    ...
    interface order {
        ...
        public static String getOrderByUserId = "select UserName, PhoneNumber, Email, o.OrderID, o.Payment, o.OrderDate, o.ShipDate, o.ReceivedDate, o.Status, c.Quantity, p.ProductID, p.ProductName, p.[Image], p.Price   from ORDERS o inner join CART c on o.OrderID= c.OrderID\n"
                + "inner join ACCOUNT a on a.UserID= c.UserID\n"
                + "inner join SIZE s on s.SizeID= c.SizeID\n"
                + "inner join PRODUCTS p on p.ProductID= s.ProductID\n"
                + "where s.AgentID=?";
        public static String updateStatusAndRecievedDateByOrderId= "Update ORDERS Set Orders.[Status]= ? , ReceivedDate= GETDATE() where OrderID= ?";
        public static String updateStatusAndShipDateByOrderId= "Update ORDERS set Orders.[Status]= ? , ShipDate= GETDATE() where OrderId=?";
    }
