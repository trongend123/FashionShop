...
public class ProductDao extends MyDao implements SqlQuery {
    ...
    //  to get new product
    public List<Product> getRealateProductByProductId(int productId) {
        List<Product> list = new ArrayList<>();

        try {
            ps = con.prepareStatement(SqlQuery.product.getRelateProductByProductId);
    ...
    public List<String> getAllProductName() {
        ...
        try {
            ps = con.prepareStatement(SqlQuery.product.getAllProductName);
            rs = ps.executeQuery();
...
//            ps.setString(2, size);
    ...
    public List<Product> searchProductByName(String name) {
        List<Product> list = new ArrayList<>();
        String sqlx = "	SELECT * FROM [PRODUCTS] WHERE ProductName LIKE N'%" + name + "%'";
        try {
            ps = con.prepareStatement(sqlx);
//            ps.setString(1, name);
            rs = ps.executeQuery();
    ...
    // get list product from price to greater or lesthan
    public List<Product> filterListInSearchProduct(String minPrice, String maxPrice, String productName) {
        List<Product> list = new ArrayList<>();
        String emptyPrice = "";
        // user only input minPrice 
        if (minPrice.equalsIgnoreCase(emptyPrice) == false && maxPrice.equalsIgnoreCase(emptyPrice) == true) {
            list = searchProductByMinPrice(minPrice, productName);
            return list;
        } // user only input maxPrice 
        else if (maxPrice.equalsIgnoreCase(emptyPrice) == false && minPrice.equalsIgnoreCase(emptyPrice) == true) {
            list = searchProductByMaxPrice(maxPrice, productName);
            return list;
        }// user input both fileds 
        else if (maxPrice.equalsIgnoreCase(emptyPrice) == false && minPrice.equalsIgnoreCase(emptyPrice) == false) {
            list = searchProductByMinAndMaxPrice(minPrice, maxPrice, productName);
            return list;
        }
        return list;
    }

    public List<Product> searchProductByMinPrice(String minPrice, String productName) {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT * FROM [PRODUCTS] WHERE Price > ? AND ProductName LIKE N'%" + productName + "%'";
        try {
            ps = con.prepareStatement(sql);
    ...
    public List<Product> searchProductByMaxPrice(String maxPrice, String productName) {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT * FROM [PRODUCTS] WHERE Price < ? AND ProductName LIKE N'%" + productName + "%'";
        try {
            ps = con.prepareStatement(sql);
    ...
    public List<Product> searchProductByMinAndMaxPrice(String minPrice, String maxPrice, String productName) {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT * FROM [PRODUCTS]  WHERE Price BETWEEN ? and ? AND ProductName  LIKE N'%" + productName + "%'";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, minPrice);
    ...
    public List<Product> getNewProductByName(String productName) {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT * FROM [PRODUCTS]  WHERE ProductName  LIKE N'%" + productName + "%' ORDER BY ProductID DESC";
        try {
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            //get all product by name
...
//        }
        List<Product> list = a.getRealateProductByProductId(1);
